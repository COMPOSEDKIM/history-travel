...
function initMap() {
  ...
  // let testGET = '["[[{\\"lat\\":34.6872571,\\"lng\\":135.5258546}],[{\\"lat\\":34.6872571,\\"lng\\":135.5258546}]]"]';
  // //            '["[{\"lat\":34.6872571,\"lng\":135.5258546}]","[{\"lat\":34.6872571,\"lng\":135.5258546}]"]';
  // let jsonArray = JSON.parse(testGET);
  // let test1 = jsonArray.map(item => JSON.parse(item));
  //[[{"lat":34.6842971,"lng":135.5263235}]]
  //[["호코쿠 신사","오사카 성"]]

  let testGET = '[[{\"lat\":34.6872571, \"lng\":135.5258546},{\"lat\":34.6842971, \"lng\":135.5263235}],[{\"lat\":34.6872571, \"lng\":135.5258546}],[{\"lat\":34.6872571, \"lng\":135.5258546}]]';
  let jsonArray = JSON.parse(testGET);
  let test1 = jsonArray.map(innerArray => innerArray.map(obj => ({ lat: obj.lat, lng: obj.lng })));

  let placenameGET = '[[\"오사카 성\",\"호코쿠 신사\"],[\"오사카 성\"],[\"오사카 성\"]]'
  let jsonArray1 = JSON.parse(placenameGET);
  let placename = jsonArray1.map(innerArray => innerArray.map(str => str.replace(/\"/g, '')));
  let markerlist = [[]];    
  const wayps = []
  // let parsedArray = JSON.parse(placenameGET)[0];
  // let cleanedString = parsedArray.replace(/\\"/g, '"');
  // let placename = JSON.parse(cleanedString);
 // let test1 = testGET.map(item => JSON.parse(item));
 // let placename = placenameGET.map(item => JSON.parse(item));
